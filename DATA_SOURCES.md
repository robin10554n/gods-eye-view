...
- **TfL JamCams.** The camera list comes from the keyless `api.tfl.gov.uk` endpoint (an optional `TFL_APP_KEY` raises its rate limit); JPEG stills and short MP4 clips come from TfL's public S3 bucket. Thumbnail cards use the stills; the focused monitor plane plays the MP4 when TfL publishes one. These are looping clips updated every few minutes, not a 24/7 stream. The "Powered by TfL Open Data" attribution is required by TfL's terms and is registered in the Data attribution popover.
